...
int main() {
  ...
  ScalarEvolution xe(NAN);
  ScalarEvolution lon(1.0, 2.0), lat(-1.1, 2.1), r(1.2, -2.2);

  if(!test.check("invalid(time)", !HorizontalTrack(Time::undefined(), Interval(1.0), lon, lat, r).is_valid())) n++;
